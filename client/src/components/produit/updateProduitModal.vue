...
export default {
  ...
  props: ["produit", "title", "disableButton"],

  watch: {
    dialog: function() {
      this.stock = this.produit.stock;
      this.declaration = this.produit.declaration;
      this.prixVente = this.produit.prixVente;
      this.prixAchat = this.produit.prixAchat;
    },
  },
  methods: {
    async update() {
      ...
      try {
        ...
        this.lading = false;
        this.dialog = false;
        this.text = `Successfully saved the new Produit ${this.declaration} `;
        this.snackbar = true;
      } catch (err) {
        console.log(err);
        this.lading = false;
